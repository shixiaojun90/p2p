    ...
    */
    public function index()
    {
		...
		foreach($list as $key=>$v){
			...
			//
			$invite_money = M('invite_awards')->where("uid={$uid}")->sum('affect_money');//邀请奖励
			$list[$key]['invite_money'] = $invite_money;
			//
...
public function export(){
		...
		foreach($list as $key=>$v){
			...
			$list1[$key] = get_personal_benefit($uid);
			$list2[$key] = get_personal_out($uid);
			...
			$money['jzlx'] = $list1[$key]['interest'];//净赚利息
			$money['jflx'] = $list2[$key]['interest'];//净付利息
			#$money['jflx'] = $borrowANDpaid[0]['interest'];//净付利息
			$money['ljjj'] = $umoney['reward_money'];//累计收到奖金
			$money['ljhyf'] = $uplevefee;//累计支付会员费
			$money['ljtxsxf'] = $list2[$key]['withdraw_fee'];//累计提现手续费  withdraw_money
			...
			$money['ljtxje'] = $list2[$key]['withdraw_money'];//累计提现金额
			$money['ljzfyj'] = $commission1 + $commission2;//累计支付佣金
			$money['glycz'] = $listarray['17']['money'];//管理员操作资金
		//
		...
		$row=array();
		$row[0]=array('ID','用户名','真实姓名','总余额','可用余额','冻结金额','待收本息金额','待收本金金额','待收利息金额','待付本息金额','待付本金金额','待付利息金额','待确认投标','累计提现手续费','累计提现金额','累计充值金额','累计支付佣金','累计投标奖励','累计推广奖励','净赚利息','净付利息','管理员操作资金');
		$i=1;
		foreach($list as $v){
				$row[$i]['uid'] = $v['id'];
