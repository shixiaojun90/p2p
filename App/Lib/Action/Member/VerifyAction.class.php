...
//		} 
		...
		if($newid){
		    ...
		    $is_verifyname=$datag['is_verifyname'];
			
		    if($is_verifyname=='1'){  //开启实名验证
			    import("ORG.Loan.Escrow");
			    $loan = new Escrow();
			    $notify =C('WEB_URL').U("member/Notify/verifyIdCard");
			    $res=$loan->verifyName(array('realName'=>$data['real_name'],'identificationNo'=>$data['idcard']),$notify);
				//file_put_contents('result.txt', $res);
			    $returnarray= json_decode($res,true);
			    if($returnarray['ResultCode']==88){
				    ajaxmsg();
			    }else{
				    ajaxmsg('身份证验证不通过,请确认后重新填写',0);
			    }
			}else{
			  ajaxmsg();
			}
