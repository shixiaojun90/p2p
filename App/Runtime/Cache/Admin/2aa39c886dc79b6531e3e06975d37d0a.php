...
<!--搜索-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td><?php echo ($vo["withdraw_money"]); ?>元</td>
		...
		<td> <?php if($vo["withdraw_status"] == 4): ?><a target="_blank" href="<?php echo U('Audit',array('id'=>$vo['id'],'status'=>5));?>">提现通过</a> &nbsp; <a target="_blank" href="<?php echo U('Audit',array('id'=>$vo['id'],'status'=>6));?>">提现退回</a><?php else: ?>--<?php endif; ?></td>
