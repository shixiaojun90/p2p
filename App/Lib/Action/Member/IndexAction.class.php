...
class IndexAction extends MCommonAction {
    public function index(){ 
		...
		//检测投资授权和借款授权
		$escrow_account = M("escrow_account")->field("invest_auth,repayment")->where("uid = {$this->uid}")->find();
		$this->assign("escrow_account",$escrow_account);
	...
	public function shouquan(){
		...
		if($type=="t"){
			...
			checkAuthorize($this->uid,$auth); //检测借款授权
		}
	}
