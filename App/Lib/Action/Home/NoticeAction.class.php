		...
		public function withdraw()
	{
        ...
        if($loan->withdrawVerify($_POST)){
		     ...
		     if($_POST['ResultCode']=='88' && !$vo['withdraw_status']){ // 成功
                ...
                $updata['fee_max'] = $_POST['FeeMax'];
                $updata['fee_percent'] = $_POST['FeePercent'];
                $updata['fee']  = $_POST['Fee'];
                $updata['free_limit'] = $_POST['FreeLimit'];
                $updata['fee_rate'] = $_POST['FeeRate'];
                $updata['fee_splitting'] = $_POST['FeeSplitting'];
                
                $xid = M('member_withdraw')->where("uid={$vo['uid']} AND id={$id}")->save($updata);
				
                if($xid){
                    $amoney=$_POST['Amount']-$_POST['FeeWithdraws'];
                    memberMoneyLog($uid,29,$_POST['Amount'],"提现成功,扣除实1际手续费".$FeeWithdraws."元，到帐金额".$amoney."元",'0','@网站管理员@',0);
					$um = M("members")->field("user_phone,user_name")->find($uid);
					
					SMStip("withdraw",$um['user_phone'],array("#USERANEM#","#MONEY#"),array($um['user_name'],$amoney));
					//SMStip("withdraw",$um['user_phone'],"提现成功");
                    MTip('chk6',$uid);
                    echo "SUCCESS";
            ...
            }elseif($_POST['ResultCode']=='90' && !$vo['withdraw_status']){ //待审核
                $FreeLimit=$_POST['FreeLimit'];
                $FeeWithdraws=$_POST['FeeWithdraws'];
                $Amount=$_POST['Amount'];
            
                $updata['withdraw_status'] = 4;
                $updata['second_fee'] = $_POST['FeeWithdraws'];
                $updata['withdraw_fee'] = $FeeWithdraws;
                $updata['loanno'] = $_POST['LoanNo'];
                ...
                if($xid){
                    $amoney=$_POST['Amount']-$_POST['FeeWithdraws'];
                    memberMoneyLog($uid,4,-$Amount,"提现审核中,预扣除".$Amount.'元','0','@网站管理员@',0);
                    MTip('chk6',$uid);
                    echo "SUCCESS";
                    notifyMsg('提现待审核',$_POST, 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], 'SUCCESS');
                    exit;
                }
            }
