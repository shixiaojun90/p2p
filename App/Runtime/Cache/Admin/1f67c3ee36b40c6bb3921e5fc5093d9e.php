...
<!--搜索/筛选会员-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td><?php if($vo['status'] == 0): ?><span style="color:#F00"><?php if(($vo['loanno'] == '') and ($vo['vip_fee'] != '0.00')): ?>未支付<?php else: ?>待审核<?php endif; ?></span><?php elseif($vo['status'] == 1): ?><span style="color:#090">审核通过</span><?php else: ?>审核未通过<?php endif; ?></td>
        <td>
            <?php if($vo['status'] == 0): if(($vo['loanno'] == '') and ($vo['vip_fee'] != '0.00')): ?>--<?php else: ?>
            <a href="__URL__/edit?id=<?php echo ($vo['id']); ?>">审核</a><?php endif; ?>
            <?php else: ?>
            --<?php endif; ?>
        </td>
