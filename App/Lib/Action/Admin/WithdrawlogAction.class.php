...
class WithdrawlogAction extends ACommonAction
{
    ...
    */
	...
	public function _doEditFilter($m){
		...
		//file_put_contents("vom.txt",M()->getlastSql());
		if($vox['withdraw_status']<>3 && $m->withdraw_status==3){
			...
			//file_put_contents("um.txt",M()->getlastSql());
...
/**
	...
	*/
    ...
    public function Audit()
    {
        ...
        $loan = new Escrow();
        $loan_no_list = M("member_withdraw")->where("id={$id}")->getfield('loanno');
        $returnURL = 'http://'.$_SERVER['HTTP_HOST'].U("auditreturn");
        $notifyURL = 'http://'.$_SERVER['HTTP_HOST'].U("Home/Notify/withdrawAudit");
        $data = $loan->loanAudit($loan_no_list, $status, $returnURL, $notifyURL, '提现审核');
