...
class BorrowAction extends ACommonAction
{
    ...
    */
    public function waitverify()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.updata,b.borrow_money,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,b.is_tuijian,b.money_collect';
    ...
    public function waitverify2()
    {
		...
		$map['b.borrow_type'] = array('not in','6,7');
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.full_time,m.user_name,m.id mid,b.is_tuijian,b.money_collect';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function waitmoney()
    {
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,b.is_tuijian,b.has_borrow,b.money_collect';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function repaymenting()
    {
		...
		$field= 'm.id as mid,m.customer_name,b.id,b.batch_no,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.borrow_interest,b.repayment_money,b.repayment_interest,b.borrow_interest_rate,b.repayment_type,b.deadline,m.user_name,m.user_phone,b.is_tuijian,b.money_collect';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    {//暂时未处理
		...
		$field= 'id,borrow_name,borrow_uid,borrow_duration,borrow_type,borrow_money,repayment_money,updata,borrow_interest_rate,repayment_type,deadline';
	...
	public function unfinish(){
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.deadline,m.id mid,m.user_name,v.deal_user_2,v.deal_time_2,v.deal_info_2';
    ...
    public function done()
    {
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.repayment_money,b.deadline,m.id mid,m.user_name';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function fail()
    {
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,v.deal_user,v.deal_time,m.id mid,v.deal_info';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->join("{$this->pre}borrow_verify v ON b.id=v.borrow_id")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function fail2()
    {
		...
		$field= 'b.id,b.batch_no,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,v.deal_user_2,v.deal_time_2,v.deal_info_2';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->join("{$this->pre}borrow_verify v ON b.id=v.borrow_id")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function _editFilter($id)
    {
		...
		switch(strtolower(session('listaction'))){
			...
			case "waitverify2":
				...
				$borrow_fee = M("borrow_investor")->field('sum(borrow_fee) as  borrow_fee')->where("borrow_id={$_GET['id']}")->find();
                $this->assign('borrow_fee', $borrow_fee['borrow_fee']);
			break;
			case "waitmoney":
				for($i=0;$i<=10;$i++){
		...
		$sql ="select id,title from lzh_article where type_id =7";
		$danbao = M()->query($sql);
		$dblist = array();
		if(is_array($danbao)){
	...
	public function doEditWaitverify(){
		...
		$vm = M('borrow_info')->field('borrow_uid,borrow_status,borrow_type,first_verify_time,password,updata,danbao,money_collect')->find($m->id);
	...
	public function doEditWaitverify2(){
		...
		$vm = M('borrow_info')->field('borrow_uid,borrow_money,borrow_status,first_verify_time,updata,danbao,borrow_interest_rate,borrow_duration,repayment_type,collect_day,collect_time,money_collect')->find($m->id);            
		if($m->borrow_status<>5 && $m->borrow_status<>6){
			$this->error('已经满标的的借款只能改为复审通过或者复审未通过');
			exit;
		}
		//复审投标检测
		$capital_sum2=M('borrow_investor')->where("borrow_id={$m->id} and loanno <> ''")->sum('investor_capital');
        ...
        $loan = new Escrow();
        $loan_no_list = getInvestLoanNo($_POST['id']);
		//print_r($loan_no_list);exit;
		if($_POST['borrow_status']==6){//复审通过

            $returnURL = 'http://'.$_SERVER['HTTP_HOST'].U("auditreturn");
            $notifyURL = 'http://'.$_SERVER['HTTP_HOST'].U("Home/Notify/audityes");
            $data = $loan->loanAudit($loan_no_list, 1, $returnURL, $notifyURL, '复审通过');
			//print_r($data);exit;
            $form =  $loan->setForm($data, 'audit');
            echo $form;
            exit;
		}elseif($_POST['borrow_status']==5){//复审未通过

            $returnURL = 'http://'.$_SERVER['HTTP_HOST'].U("auditreturn");
            $notifyURL = 'http://'.$_SERVER['HTTP_HOST'].U("Home/Notify/auditno");
            $data = $loan->loanAudit($loan_no_list, 2, $returnURL, $notifyURL, "复审未通过(管理员{$this->admin_id})");
            $form =  $loan->setForm($data, 'audit');
            echo $form;
            exit;
	...
	public function doEditWaitmoney(){
		...
		$vm = M('borrow_info')->field('borrow_uid,borrow_type,borrow_money,first_verify_time,borrow_interest_rate,borrow_duration,repayment_type,collect_day,collect_time,money_collect,borrow_uid')->find($m->id);
		...
		if($m->borrow_status==3){ 
			import("ORG.Loan.Escrow");
            $loan = new Escrow();
            $loan_no_list = getInvestLoanNo($borrow_id );
			if(!$loan_no_list){
				//流标返回
                    ...
                    MTip('chk11',$vm['borrow_uid'],$borrow_id);
                    $vss = M("members")->field("user_phone,user_name")->where("id = {$vm['borrow_uid']}")->find();
                    SMStip("refuse",$vss['user_phone'],array("#USERANEM#","ID"),array($vss['user_name'],$borrow_id));
                    ...
                    if ($result = M('borrow_info')->where("id={$borrow_id}")->save($borrow_save)) { //保存成功
                            ...
                            $verify_info['deal_info_2'] = '流标处理';
                            $verify_info['deal_user_2'] = $this->admin_id;
                            $verify_info['deal_time_2'] = time();
                            $verify_info['deal_status_2'] = 3;
                            if($vm['first_verify_time']>0) M('borrow_verify')->save($verify_info);
                            else  M('borrow_verify')->add($verify_info);
                            $this->success("处理完成", U("waitmoney"));
							exit;
                    }
			}else{
				$returnURL = C('WEB_URL').U("auditreturn");
				$notifyURL = C('WEB_URL').U("Home/Notify/auditBids");
				$data = $loan->loanAudit($loan_no_list, 2, $returnURL, $notifyURL, "流标处理(管理员{$this->admin_id})");
				$form =  $loan->setForm($data, 'audit');
				echo $form;
				exit;
			}
	...
	public function _listFilter($list){
		...
		foreach($list as $key=>$v){
			...
			$v['invest_num'] = M('borrow_investor')->where("borrow_id={$v['id']} and loanno <>''")->count(id);//已投资纪录数量
			$row[$key]=$v;
		}
		return $row;
	}
	 ...
	 public function doinvest()
    {
		...
		$field= 'bi.id bid,b.id,bi.investor_capital,bi.investor_interest,bi.invest_fee,bi.borrow_fee,bi.add_time,bi.is_auto,m.user_name,m.id mid,m.user_phone,b.borrow_duration,b.repayment_type,m.customer_name,b.borrow_type,b.borrow_name';
