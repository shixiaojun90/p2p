<?php
   ...
   class DebtBehavior
   {
       ...
       */
        ...
        */
        ...
        {  
            ...
            $transfer = M('borrow_investor i')
                ...
                ->field("i.id, i.borrow_id, i.add_time, i.deadline, i.investor_interest, i.investor_capital,b.borrow_type, b.borrow_name, b.borrow_interest_rate, m.user_name")
        ...
        */
        public function sell($invest_id, $price)
        {
            $invest_id = intval($invest_id);
            $price = floatval($price);
            $check = $this->checkSell($invest_id, $price);  
            if($check==='TRUE'){ // 检测通过
        ...
        */
        private function checkSell($invest_id, $price)
        {
            $condition = "invest_id= '".$invest_id."' and status in (6,7)";
            ...
            $vm = getMinfo($this->uid,'mm.account_money,mm.back_money'); 
        ...
        */
        ...
        {    
            ...
            $Bonds['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id, d.status, i.borrow_id, d.money, d.transfer_price, d.addtime, d.period,b.borrow_type, d.total_period, b.borrow_name, b.borrow_interest_rate, b.total, b.has_pay")
        ...
        */
        public function cancel($invest_id)
        {
            $invest_id = intval($invest_id);  
        ...
        */
        ...
        {  
            ...
            $Bonds['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->field("d.invest_id, d.remark,b.borrow_type, i.borrow_id, d.money, d.transfer_price, d.cancel_time, 
        ...
        {  
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id, i.borrow_id, d.money, d.transfer_price, d.buy_time, d.status, m.user_name,  
                            d.cancel_times, d.period, d.total_period, d.serialid, b.borrow_name,b.borrow_type, b.borrow_interest_rate, b.total, b.has_pay")
        ...
        */
        public function buyDetb()
        {
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id, i.borrow_id, d.money, d.transfer_price,b.borrow_type, d.buy_time, d.status, d.serialid, m.user_name,  
                            d.cancel_times, d.period, d.total_period, b.borrow_name, b.borrow_interest_rate, b.total, b.has_pay")
        ...
        public function onDetb()
        {
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id, i.borrow_id, d.money, d.serialid, m.user_name,  
                            d.total_period, b.borrow_name,b.borrow_type, b.total, b.has_pay")
        ...
        */
        public function listAll($condition, $size=0, $pstatus=true)
        {
            !$condition && $condition = "d.status in (2, 4,1)";
            ...
            $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,
                     i.investor_uid, i.deadline, b.id, b.borrow_name,b.borrow_type, b.borrow_interest_rate, m.credits, m.user_name";
            if($pstatus){ 
                //$count = M("invest_detb as d")->join("{$this->pre}members m ON i.investor_uid=m.id")->where("status in (2, 4)")->count();
				$count = M("invest_detb d")
                        ->join("{$this->pre}borrow_investor i ON d.invest_id=i.id")
                        ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        * @return  boolean 
        */
        public function buy($invest_id)
        {
            $check_result = $this->checkBuy($invest_id);
            if($check_result === 'TRUE'){
                ...
                if($investor_status && $detail_status && $debt_status && $sell_user_status && $buy_user_status){
                    $this->Model->commit();
                    $this->moneyLog($this->uid, 45, -$debt_info['transfer_price'], $debt_info['money'], "购买{$serialid}号债权", $debt_info['sell_uid']);
                    $this->moneyLog($debt_info['sell_uid'], 46, $debt_info['transfer_price'], $debt_info['money'], "转让{$serialid}号债权", $this->uid);
                    memberMoneyLog($debt_info['sell_uid'],47,-$fee,"转让{$serialid}号债权手续费（转让金额的{$debt_fee}%）");
                    //return 'TRUE';  
                    return '购买成功';
                }else{
                    $this->Model->rollback();
        ...
        * @return  boolean 
        */
        public function checkBuy($invest_id)
        {
        ...
        */
        public function adminList($status='1')
        {
            ...
            $list['data'] = M("invest_detb d")
                ...
                ->join("{$this->pre}borrow_info b ON i.borrow_id= b.id")
                ->join("{$this->pre}members m ON d.sell_uid=m.id")
                ->field("d.id as debt_id,b.borrow_type,d.remark, d.cancel_time, d.invest_id, d.status, m.user_name, b.borrow_name, b.id, d.addtime, b.borrow_interest_rate, b.total, b.has_pay, d.period,b.borrow_type, d.transfer_price")
                ->where("1 and {$status}")
                ->limit($Page->firstRow .','. $Page->listRows) 
        ...
        * 
        */
        public function qddBuy($invest_id)
        {
            $info = array();
            $debt_info = $this->Model->field("transfer_price, sell_uid, money, status")->where("invest_id={$invest_id}")->find();
			if($debt_info['status']!=2){
				return '转让结束';
			}
            $buy_user = M("member_money")->field("account_money, back_money, money_collect")->where("uid={$this->uid}")->find();
            $sell_user = M("member_money")->field("account_money, money_collect")->where("uid={$debt_info['sell_uid']}")->find();
            ...
            $datag = get_global_setting();
            $debt_fee = $datag['debt_fee'];// 百分比
            $info['fee'] = round(($debt_info['transfer_price'] * $debt_fee)/100, 2);  // 转让手续费
            //转让方增加资金
            $sell_user['account_money'] += $debt_info['transfer_price'];
            $sell_user['money_collect'] = $sell_user['money_collect'] - $debt_info['money'];// 减去待收
            ...
            // 转让序列号
            $info['serial'] = "ZQZR-".date("YmdHis").'-'.$invest_id;
            $info['money'] = $debt_info['transfer_price'];// 转让价格
			if($this->Model->where("invest_id={$invest_id}")->save(array('serialid'=>$info['serial']))){
				return $info; 
			}else{
				return '购买失败';
			}
        ...
        * 
        */
        public function qddUpdate($invest_id, $serial, $loanno, $buy_uid, $sell_uid)
        {
			$buy_uid = intval($buy_uid);
			$sell_uid = intval($sell_uid);
            $invest_id = intval($invest_id);
            $debt_info = $this->Model->field("transfer_price, sell_uid, money, status")->where("invest_id={$invest_id}")->find();
            ...
            $buy_user = M("member_money")->field("account_money, back_money, money_collect")->where("uid={$buy_uid}")->find();

            $sell_user = M("member_money")->field("account_money, money_collect")->where("uid={$sell_uid}")->find();
            ...
            $this->Model->startTrans();
            $investor_status = M("borrow_investor")->where("id={$invest_id}")->save(array("debt_status" => 2, 'debt_uid'=>$buy_uid));
            $detail_status = M("investor_detail")->where("invest_id={$invest_id} and status in (6,7)")->save(array('investor_uid'=>$buy_uid));
            $debt_status = $this->Model->where("invest_id={$invest_id}")->save($debt);
            $sell_user_status = M("member_money")->where("uid={$sell_uid}")->save($sell_user);
            $buy_user_status = M("member_money")->where("uid={$buy_uid}")->save($buy_user);
            if($investor_status && $detail_status && $debt_status && $sell_user_status && $buy_user_status){
                $this->Model->commit();
                $this->moneyLog($buy_uid, 46, -$debt_info['transfer_price'], $debt_info['money'], "购买{$serial}号债权", $sell_uid);
                $this->moneyLog($sell_uid, 47, $debt_info['transfer_price'], $debt_info['money'], "转让{$serial}号债权", $buy_uid);
                memberMoneyLog($sell_uid,48,-$fee,"转让{$serial}号债权手续费（转让金额的{$debt_fee}%）");
