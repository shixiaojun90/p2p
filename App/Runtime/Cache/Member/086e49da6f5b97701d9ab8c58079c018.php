    ...
    <div style="width:700px; margin:30px auto; text-align:center; float:left; margin-left:150px;">
      <div id=qh_conn0 >
        ...
        <div style="width:600px; float:left; margin-top:20px;">
          <input type="image" src="__ROOT__/Style/H/images/xiayibu.jpg" class="xiayibu" onclick="sendMobileValidSMSCode();" />
        ...
        <div style="width:600px; margin-left:100px;">
          <table width="600" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="289"><input type="text" id="txt_email" class="register_email" style="color:#969696;"  onfocus="if(this.value=='请输入电子邮件地址') {this.value='';}" onblur="if(this.value=='') {this.value='请输入电子邮件地址';}" value="<?php echo session('email_temp');?>"/></td>
              <td width="150"><div id="sendSMSTip" class="dv_r_4_3"> </div></td>
            </tr>
          </table>
        </div>
		...
		$.ajax({
			...
			success: function(d) {
				...
				if (d.status == 1) {
					...
					$.jBox("get:/member/common/phone?phone="+mobile, {title:"短信获取验证码"});
					clearInterval(timer);
...
function code(){
	var mobile = "18678578692";
    $.jBox("get:__APP__/member/Verify/phone?phone="+mobile, {
	    title:"短信获取验证码",
		width:"600",
		buttons: {'关闭': true }
	});
