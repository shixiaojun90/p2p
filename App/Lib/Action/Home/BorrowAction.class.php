...
class BorrowAction extends HCommonAction {
	...
	public function post(){
		...
		$auth=array('repayment','secondary_percent');
		checkAuthorize($this->uid,$auth);
        $bank = M('member_banks')->field("bank_num")->where("uid={$this->uid}")->find();
        if(!$bank['bank_num']){
		   $this->error('请先绑定银行卡后再发布借款', '/member/bank#fragment-1');	
		}
   
		$vminfo = M('members')->field("user_leve,time_limit,is_borrow,is_vip")->find($this->uid);
		if($vminfo['is_vip']==0){
			$_xoc = M('borrow_info')->where("borrow_uid={$this->uid} AND borrow_status in(0,2,4) and borrow_type not in (6,7)")->count('id');
	...
	public function save(){
	   $ispass=intval($_POST['is_pass']);
		if($ispass==1 && empty($_POST['password'])){
			$this->error("您发布的是定向标,需要输入密码,请重新发布");
			exit();
		}
		if(!$this->uid) $this->error("请先登陆",__APP__."/member/common/login");
		$pre = C('DB_PREFIX');
		...
		//相关的判断参数
		$borrow['borrow_type'] = $_POST['vkey'];//intval(cookie(text($_POST['vkey'])));
		...
		switch($borrow['borrow_type']){
			case 1://普通标
				if($_minfo['credit_cuse']<$borrow['borrow_money']) $this->error("您的可用信用额度为{$_minfo['credit_cuse']}元，小于您准备借款的金额，不能发标,请前往申请",__APP__."/member/moneylimit#fragment-1");
			break;
			case 2://新担保标
			break;
			case 4://净值标
        ...
        $borrow['batch_no'] = build_order_no();
