...
class WelcomeAction extends ACommonAction {
    ...
    public function index(){
		...
		$row['borrow_2'] = M('borrow_info')->where('borrow_status=4')->count('id');//普通标复审
		...
		for($i=0; $i<7; $i++){
			...
			$arr = M("investor_detail")->field(" sum(interest) as interest, sum(capital) as capital")->where("pay_status = 1 and status=7 and $deadline")->find();
			$arr['deadline'] = $end;
