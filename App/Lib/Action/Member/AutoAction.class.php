...
class AutoAction extends MCommonAction {
    ...
    public function auto(){
		...
		if($vot){
			$votcount = M()->query("SELECT count( `id` ) tnum FROM `lzh_auto_borrow` WHERE `borrow_type` =6
				...
				SELECT `invest_time` 
				FROM `lzh_auto_borrow` 
				WHERE borrow_type=6 and uid ={$this->uid})"); 
		}
        $this->assign("tnum",$vtcount['0']['tnum']);
		$this->assign("vot",$vot);
		...
		if($vod){
			$vodcount = M()->query("SELECT count( `id` ) tnum FROM `lzh_auto_borrow` WHERE `borrow_type` =7
				...
				SELECT `invest_time` 
				FROM `lzh_auto_borrow` 
				WHERE borrow_type=7 and uid ={$this->uid})"); 
		}
    ...
    public function autolong(){
		$vo = M('auto_borrow')->where("uid={$this->uid}")->select();
		//dump($vo);exit;
		//$vo[0]['is_use_name'] = ($vo['is_use']==0)?"当前设置已暂停使用":"当前设置已启用";
