        ...
        public function audit()
        {
            if($_POST['dosubmit']){
                ...
                if($status == 2){
                    ...
                    if(!$result = M("invest_detb")->where("id={$debt_id}")->save($data)){
                        $this->error("审核失败", U("debt/index"));
                    }
                }elseif($status == 3){
                    $debt_info = M("invest_detb")->field("invest_id")->where("id={$debt_id}")->find();
                    M("invest_detb")->where("id={$debt_id}")->save($data);
