<?php
    ...
    */
    ...
    class Escrow
    {
        ...
        * 
        */
        public function registerAccount( $mobile, $email, $realname, $identification_no,  $loan_plat_form_account, $register_type=2, $account_type='', $image1='', $image2='',$remark1='', $remark2='', $remark3='')
        {
		...
		*/
		public function withdraw($arr){
			...
			$dataStr = array('WithdrawMoneymoremore','PlatformMoneymoremore','OrderNo','Amount','FeePercent','FeeMax','FeeRate','CardNo','CardType','BankCode','BranchBankName','Province','City','RandomTimeStamp','Remark1','Remark2','Remark3','ReturnURL','NotifyURL');
        ...
        */
        ...
        {   
            ...
            $form =  '
            <form method="post" name="Forms" id="binding" action="'.$this->url_arr[$url_k].'">';
    ...
    * @return string
    */
    public function Moneymoremorecharge ($RechargeMoneymoremore, $PlatformMoneymoremore, $OrderNo, $Amount, $ReturnURL, $NotifyURL, $RechargeType='', $FeeType='', $CardNo='',  $Remark1='', $Remark2='', $Remark3='') 
    {
    ...
    */
    ...
    { 
	  ...
	  *实名匹配加签
      */	 
     public function verifyName($datainfo,$notify) {
            $data['PlatformMoneymoremore'] = $this->plat_form_money_moremore;          
            $data['IdentityJsonList'] = $this->IdentityJsonList($datainfo);
            $data['RandomTimeStamp'] = $this->getRandomTimeStamp();;      
            $data['NotifyURL'] = $notify;      
            $dataStr = implode('',$data);
			//file_put_contents("11111.txt",$data['IdentityJsonList']);
            if($this->antistate == 1)
            {
                $dataStr = strtoupper(md5($dataStr));
            }
			...
			file_put_contents('result123.txt', var_export($data,true));
            return $this->postDate($data,$this->url_arr['verifyname']);
    } 
    
     /**
	  *实名匹配验签
      */
    public function verifyNameVerify($data) {
            $dataStr = implode('',$data);
            if($this->antistate == 1)
            {
               $dataStr = strtoupper(md5($dataStr));
             ...
             $SignInfo =  $this->rsa->sign($dataStr);
			 //file_put_contents('asdas.txt',$dataStr);
            return  $this->rsa->verify($dataStr,$SignInfo);
    }
	/**
	 *实名匹配列表
     */
    public function IdentityJsonList($data) {
        $data['realName']           =$data['realName'];
        $data['identificationNo']   =$data['identificationNo'];
        return json_encode($data);
    }

	//微信端

	public function registerAccountphone( $mobile, $email, $realname, $identification_no,  $loan_plat_form_account, $register_type=2, $account_type='', $image1='', $image2='',$remark1='', $remark2='', $remark3='')
        {
             $data['RegisterType'] = $register_type; 
             $data['AccountType'] = $account_type;
             $data['Mobile'] = $mobile;
             ...
             $data['ReturnURL'] = C('WEB_URL').U("m/pub/regBindReturn");//返回地址  
             $data['NotifyURL'] = C('WEB_URL').U("member/Notify/bind");// 后台通知地址 
		...
		/*****微信版专用申请sq账号 ***/
	     public function registerMobileAccount( $mobile, $email, $realname, $identification_no,  $loan_plat_form_account, $register_type=2, $account_type='', $image1='', $image2='',$remark1='', $remark2='', $remark3='')
        {
             $data['RegisterType'] = $register_type; 
             $data['AccountType'] = $account_type;
             $data['Mobile'] = $mobile;
             ...
             $data['ReturnURL'] = C('WEB_URL').U("/m/bank/bindReturn");//返回地址  
             $data['NotifyURL'] = C('WEB_URL').U("member/Notify/bind");// 后台通知地址  
		...
		public function mobilewithdraw($arr){
			
			$arr['ReturnURL'] = C('WEB_URL')."/m/bank/withdrawreturn";//返回地址
			$arr['NotifyURL'] = C('WEB_URL')."/notice/withdraw"; // 通知地址
			$dataStr = array('WithdrawMoneymoremore','PlatformMoneymoremore','OrderNo','Amount','FeePercent','FeeMax','FeeRate','CardNo','CardType','BankCode','BranchBankName','Province','City','RandomTimeStamp','Remark1','Remark2','Remark3','ReturnURL','NotifyURL');
			foreach($dataStr as $v){
			    $str .= "$arr[$v]";
