...
<div class="so_main">
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td><?php echo (cnsubstr($vo["data_name"],15)); ?>(<?php if($vo["data_url"] == 2 ): ?><span style="color:red;">没有权限</span><?php elseif($vo["data_url"] != '' ): ?><a class="artZoom" href="/<?php echo ($vo["data_url"]); ?>">查看</a><?php else: ?><span style="color:red;">图片上传失败</span><?php endif; ?>)</td>
        ...
        <td><a href="javascript:void(0);" onclick="edit('?id=<?php echo ($vo['id']); ?>');">审核</a></td>
