...
<div class="form2">
	...
	<?php if(($escrow['repayment'] == 1) and ($escrow['secondary_percent'] == 1)): ?><dl class="lineD"><dt>是否流转会员：</dt><dd><?php $i=0;$___KEY=array ( 0 => '否', 1 => '是', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$vo["is_transfer"]==$k)||("key"=="value"&&$vo["is_transfer"]==$v)){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" /><?php } ?><label for="is_transfer_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?><span id="tip_is_transfer" class="tip">流转会员才能在添加企业直投的时候被选中</span></dd></dl><?php endif; ?>
	<dl class="lineD"><dt>是否冻结：</dt><dd><?php $i=0;$___KEY=array ( 0 => '否', 1 => '是', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$vo["is_ban"]==$k)||("key"=="value"&&$vo["is_ban"]==$v)){ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" /><?php } ?><label for="is_ban_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?><span id="tip_is_ban" class="tip">冻结后会员不能登陆，须联系客服处理</span></dd></dl>
...
<!-- 	<input name="bank_province" id="bank_province"  class="input" type="text" value="<?php echo ($vo["bank_province"]); ?>" ><span id="tip_bank_province" class="tip">如:山东省</span>	 -->	
                    <select name="bank_province" id="bank_province" style="width: 110px;" class="selectStyle">
	...
	<select name="bank_city" id="bank_city" style="width: 110px;" class="selectStyle">
...
var s = new GetAreaSelectbank('#bank_province','#bank_city','',<?php if(empty($vo['bank_province'])): ?>1<?php else: echo ($vo["bank_province"]); endif; ?>,<?php if(empty($vo['bank_city'])): ?>1001<?php else: echo ($vo["bank_city"]); endif; ?>);
</script>
