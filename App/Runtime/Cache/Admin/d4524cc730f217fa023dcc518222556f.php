...
<!--搜索/筛选会员-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td>
            <a href="__URL__/edit?id=<?php echo ($vo['id']); ?>" target="_blank">审核</a> 
            <?php $fre = ceil($vo['invest_num']/200) ?>
            <?php if($fre > 1): ?><br> <font  style="color: red; line-height :25px;"> 注：共<?php echo ($vo['invest_num']); ?>条记录需要审核<?php echo ($fre); ?>次,审核必须一致</font><?php endif; ?>
        </td>
