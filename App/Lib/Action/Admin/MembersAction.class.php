...
class MembersAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		if(!empty($_REQUEST['bj']) && !empty($_REQUEST['lx']) && !empty($_REQUEST['money'])){
			...
			if($_REQUEST['lx']=='allmoney'){
				 $_REQUEST['bj'] =preg_replace("/\s/","",$_REQUEST['bj']);
				if($_REQUEST['bj']=="gt"){
					$bj = '>';
					//echo $bj.'ddd';
    ...
    public function edit() {
		...
		$escrow = M('escrow_account')->field('repayment, secondary_percent')->where("uid={$id}")->find();
        
        $this->assign('escrow', $escrow);
		//////////////////////
        $this->assign('vo', $vo);
    ...
    */
    public function escrow()
    {
        ...
        $search['real_name'] && $map .= " and real_name like '%{$search['real_name']}%'";
        $search['id_card'] && $map .= " and id_card='{$search['id_card']}'";
        $search['mobile'] && $map .= " and mobile='{$search['mobile']}'"; 
        
        
        import("ORG.Util.Page");
        $count = M('escrow_account')->where($map)->count('uid');
        $p = new Page($count, C('ADMIN_PAGE_SIZE'));
        $page = $p->show();
        $Lsql = "{$p->firstRow},{$p->listRows}";
        $list = M("escrow_account as e")
                    ->join($pre."members as m ON e.uid=m.id ")
                    ->field("e.*, m.user_name")
                    ->where($map)
                    ->limit($Lsql)
                    ->select();
