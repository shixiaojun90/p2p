...
<!--搜索/筛选逾期借款-->
  ...
  <div class="Toolbar_inbox">
    ...
    <a onclick="dosearch();" class="btn_a" href="javascript:void(0);"><span class="search_action">搜索/筛选逾期借款</span></a>
  </div>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td>
		<a href="<?php echo (getinvesturl($vo["id"])); ?>" title="<?php echo ($vo["borrow_name"]); ?>" target="_blank"><?php echo (cnsubstr($vo["borrow_name"],"15")); ?></a></td>
		<td><?php echo ($vo["total"]); ?>个月</td>
        <td><?php echo ($vo["sort_order"]); ?>/<?php echo ($vo["total"]); ?></td>
        <td><?php echo (mydate("Y-m-d H:i",$vo["deadline"])); ?></td>
        <td><?php echo ($vo['interest'] + $vo['capital']); ?>元</td>
        <td><?php echo (($vo["breakday"])?($vo["breakday"]):0); ?></td>
        <td><?php echo (($vo["expired_money"])?($vo["expired_money"]):0); ?>元</td>
        <td><?php echo (($vo["call_fee"])?($vo["call_fee"]):0); ?>元</td>
        <td><?php if($vo["substitute_money"] > 0): ?>---<?php else: ?><a href="__URL__/doexpired?id=<?php echo ($vo["id"]); ?>&sort_order=<?php echo ($vo["sort_order"]); ?>" target="_blank">代还</a><?php endif; ?></td>
